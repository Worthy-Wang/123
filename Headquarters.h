...
protected:
    ...
    vector<WarriorPtr> _warriors;                            //创建的武士部队
    ...
    unordered_map<WarriorType, size_t> _warriorTypeAmounts;  //同一类武士对应的数量
...
public:
    ...
    size_t getWarriorTypeAmounts(WarriorType type) { return _warriorTypeAmounts[type]; }
    ...
    WeaponPtr createWeapon(size_t number); //给武士配备武器
...
public:
    ...
    void show()
    {
        cout << " " << _head->getWarriorTypeAmounts(_head->getWarriorType()) << " " << toString(_head->getWarriorType()) << " in " << toString(_head->getColor()) << " headquarter" << endl;
    }
};
